...
#define TCP_DEFAULT_RTO 200000      // [us]
#define TCP_RETRANSMIT_DEADLINE 12  // [s]
...
struct tcp_queue_entry {
  struct timeval first;  // first transmission
  struct timeval last;   // last transmission
  unsigned int rto;      // retransmission timeout [us]
  uint32_t seq;
  uint8_t flg;
  size_t len;
  uint8_t data[];
...
tcp_pcb_release(struct tcp_pcb *pcb)
{
  ...
  memset(pcb, 0, sizeof(*pcb));  // pcb->state will be TCP_PCB_STATE_FREE;
}

static struct tcp_pcb *
tcp_pcb_select(struct ip_endpoint *local, struct ip_endpoint *foreign)
{
  struct tcp_pcb *pcb, *listened;
      ...
      if (pcb->local.port == local->port)
      {
        ...
        if (pcb->state == TCP_PCB_STATE_LISTEN)
          if ((pcb->foreign.addr == IP_ADDR_ANY) && (pcb->foreign.port == 0))
            listened = pcb;  // listened with wildcard
      }
    }
  }
...
tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
  ...
  case TCP_PCB_STATE_LISTEN:
    ...
    if (TCP_FLG_ISSET(flags, TCP_FLG_SYN))
    {
      ...
      pcb->local = *local;
      pcb->foreign = *foreign;
      pcb->rcv.wnd = sizeof(pcb->buf);  // define window size
  ...
  case TCP_PCB_STATE_LAST_ACK:
    if (!seg->len)
    {
      ...
      else if ((pcb->rcv.nxt <= seg->seq) && (seg->seq < (pcb->rcv.nxt + pcb->rcv.wnd)))
        acceptable = 1;
    }
    else
    {
  ...
  case TCP_PCB_STATE_CLOSE_WAIT:
    if ((pcb->snd.una < seg->ack) && (seg->ack <= pcb->snd.nxt))
    {
       ...
       * If an acknowledgement which have been SENT and fully acknowledged
  ...
  if (TCP_FLG_ISSET(flags, TCP_FLG_FIN))
  {
    ...
    case TCP_PCB_STATE_CLOSE_WAIT:
      break;  // remain in the CLOSE_WAIT state

    case TCP_PCB_STATE_LAST_ACK:
      break;  // remain in the LAST_ACK state
    }
  }
...
AGAIN:
  ...
  if (pcb->state != TCP_PCB_STATE_ESTABLISHED)
  {
    ...
    pcb->state = TCP_PCB_STATE_CLOSED;  // close the connection
    tcp_pcb_release(pcb);
    mutex_unlock(&mutex);
    return -1;
  }
...
tcp_close(int id)
{
  ...
  case TCP_PCB_STATE_CLOSE_WAIT:
    ...
    // RFC793 says "enter CLOSING state", but it seems to be LAST-ACK state
