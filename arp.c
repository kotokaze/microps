...
// See also: https://www.iana.org/assignments/arp-parameters/arp-parameters.txt
#define ARP_HRD_ETHER 0x0001
#define ARP_PRO_IP ETHER_TYPE_IP /* 0x0800 */
...
arp_cache_alloc(void)
{
  ...
  for (entry = caches; entry < tailof(caches); entry++)
  {
    if (entry->state == ARP_CACHE_STATE_FREE)
      return entry;
    if (oldest == NULL || entry->timestamp.tv_sec < oldest->timestamp.tv_sec)
      oldest = entry;
  }
...
arp_request(struct net_iface *iface, ip_addr_t tpa)
{
  ...
  req.hdr.pro = hton16(ETHER_TYPE_IP);
  ...
  memset(req.tha, 0, ETHER_ADDR_LEN);
  memcpy(req.tpa, &tpa, IP_ADDR_LEN);
  debugf("dev=%s, len=%zu", iface->dev->name, sizeof(req));
  arp_dump((uint8_t *)&req, sizeof(req));
  return net_device_output(iface->dev, ETHER_TYPE_ARP, (uint8_t *)&req, sizeof(req), iface->dev->broadcast);
}
...
arp_reply(struct net_iface *iface, const uint8_t *tha, ip_addr_t tpa, const uint8_t *dst)
{
  ...
  memcpy(reply.spa, &((struct ip_iface *)iface)->unicast, ETHER_ADDR_LEN);
...
arp_timer_handler(void)
{
  ...
  for (entry = caches; entry < tailof(caches); entry++)
  {
    if (entry->state != ARP_CACHE_STATE_FREE && entry->state != ARP_CACHE_STATE_STATIC)
    {
      ...
      if (diff.tv_sec > ARP_CACHE_TIMEOUT_SEC)
